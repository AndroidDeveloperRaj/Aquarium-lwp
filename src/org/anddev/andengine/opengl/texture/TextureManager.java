...
public class TextureManager {
	...
	/**
	 * @param pTexture
	 *            the {@link Texture} to be loaded before the very next frame is
	 *            drawn (Or prevent it from being unloaded then).
	 * @return <code>true</code> when the {@link Texture} was previously not
	 *         managed by this {@link TextureManager}, <code>false</code> if it
	 *         was already managed.
	 */
	public synchronized boolean loadTexture(final Texture pTexture) {
	...
	/**
	 * @param pTexture
	 *            the {@link Texture} to be unloaded before the very next frame
	 *            is drawn (Or prevent it from being loaded then).
	 * @return <code>true</code> when the {@link Texture} was already managed by
	 *         this {@link TextureManager}, <code>false</code> if it was not
	 *         managed.
	 */
	public synchronized boolean unloadTexture(final Texture pTexture) {
		if (this.mTexturesManaged.contains(pTexture)) {
			/*
			 * If the Texture is loaded, unload it. If the Texture is about to
			 * be loaded, stop it from being loaded.
			 */
	...
	public synchronized void loadTextures(final Texture... pTextures) {
		for (int i = pTextures.length - 1; i >= 0; i--) {
			this.loadTexture(pTextures[i]);
		}
	}

	public synchronized void unloadTextures(final Texture... pTextures) {
		for (int i = pTextures.length - 1; i >= 0; i--) {
	...
	public synchronized void reloadTextures() {
		...
		for (final Texture texture : managedTextures) { // TODO Can the use of
														// the iterator be
														// avoided somehow?
			texture.setLoadedToHardware(false);
		}

		this.mTexturesToBeLoaded.addAll(this.mTexturesLoaded); // TODO Check if
																// addAll uses
																// iterator
																// internally!
		this.mTexturesLoaded.clear();

		this.mTexturesManaged.removeAll(this.mTexturesToBeUnloaded); // TODO
																		// Check
																		// if
																		// removeAll
																		// uses
